...
__m128 CNDF ( __m128 InputX ) 
{
    ...
    ZeroValue = _mm_sub_ps(ZeroValue, InputX);
    InputX = _mm_max_ps(ZeroValue, InputX);
    sign = _mm_cvtps_epi32(_mm_and_ps(_mm_cmpgt_ps(ZeroValue, _mm_setzero_ps()), _mm_set_ps1(1)));
    ...
    MulTerm = _mm_mul_ps(_mm_set_ps1(-0.5), MulTerm);
    ...
    xNPrimeofX = _mm_mul_ps(xNPrimeofX, _mm_set_ps1(inv_sqrt_2xPI));


    //xK2 = 0.2316419 * xInput;
    //    __m128 Mul023;
    //Mul023 =  _mm_set_ps1(0.2316419);
    xK2 = _mm_mul_ps( _mm_set_ps1(0.2316419), xInput);

    //xK2 = 1.0 + xK2;
    //__m128 Add1;
    //Add1 = _mm_set_ps1(1.0);
    xK2 = _mm_add_ps(_mm_set_ps1(1.0), xK2);

    //xK2 = 1.0 / xK2;
    xK2 = _mm_div_ps(_mm_set_ps1(1.0), xK2);
    ...
    xLocal_1 = _mm_mul_ps(xK2, _mm_set_ps1(0.319381530));

    //xLocal_2 = xK2_2 * (-0.356563782);
    //    __m128 Mul035;
    //Mul035 = _mm_set_ps1(-0.356563782);
    xLocal_2 = _mm_mul_ps(xK2_2, _mm_set_ps1(-0.356563782));

    //xLocal_3 = xK2_3 * 1.781477937;
    //    __m128 Mul178;
    //Mul178 = _mm_set_ps1(1.781477937);
    xLocal_3 = _mm_mul_ps(xK2_3, _mm_set_ps1(1.781477937));
    ...
    xLocal_3 = _mm_mul_ps(xK2_4, _mm_set_ps1(-1.821255978));
    ...
    xLocal_3 = _mm_mul_ps(xK2_5, _mm_set_ps1(1.330274429));
    ...
    //xLocal   = 1.0 - xLocal;
    xLocal = _mm_sub_ps(_mm_set_ps1(1.0), xLocal); //Add1 declared up there
                            ...
                            __m128 time, __m128i otype )
{
    ...
    //MulHalf = _mm_set_ps1 ( 0.5);
    xPowerTerm = _mm_mul_ps ( xPowerTerm, _mm_set_ps1 ( 0.5));
    ...
    NegNofXd1 = _mm_sub_ps(_mm_cvtepi32_ps(otype), NofXd1);
    ZeroValue = _mm_setzero_ps();
    ZeroValue = _mm_sub_ps(ZeroValue, NegNofXd1);
    NegNofXd1 = _mm_max_ps(ZeroValue, NegNofXd1);
    
    NegNofXd2 = _mm_sub_ps(_mm_cvtepi32_ps(otype), NofXd2);
    ZeroValue = _mm_setzero_ps();
    ZeroValue = _mm_sub_ps(ZeroValue, NegNofXd2);
    NegNofXd2 = _mm_max_ps(ZeroValue, NegNofXd2);
    
    OptionPrice = _mm_sub_ps(_mm_mul_ps(sptprice, NegNofXd1),
			     _mm_mul_ps(FutureValueX, NegNofXd2));
    //otype = _mm_cvtepi32_ps(otype);
    otype = _mm_sub_epi32( _mm_sub_epi32(_mm_set1_epi32(1), otype),
			otype);
    OptionPrice = _mm_mul_ps(OptionPrice, _mm_cvtepi32_ps(otype));
...
int bs_thread() {
  int i, j, l;
	...
	sptprice = _mm_load_ps(&data.sptprice[i]);
	strike = _mm_load_ps(&data.strike[i]);
	rate = _mm_load_ps(&data.rate[i]);
	volatility = _mm_load_ps(&data.volatility[i]);
	otime = _mm_load_ps(&data.otime[i]);
	OptionType = _mm_set_epi32(((data.OptionType[i+3] == 'P') ? 1 : 0),
